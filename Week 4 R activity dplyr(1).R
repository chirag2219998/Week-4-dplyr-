...
newdata <- select(.data = Mydata, c(1,3))
newdata 
...
onlyfemale <- filter(.data = newdata1, Gender == "Female")
onlyfemale
...
total=mutate(newdata1, total=cumsum(Freq))


#Since we have a females dataset, let's make a males dataset
#TASK: Write the function that includes only rows that are 'male'

onlymale <- filter(.data = newdata1, Gender == "Male")
onlymale
